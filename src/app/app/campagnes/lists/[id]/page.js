...
export default function ListDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!user) { router.push('/login?return=' + encodeURIComponent(`/app/campagnes/lists/${listId}`)); return; }
  ...
  if (error && !list) return (
    <div className="min-h-screen bg-surface-base flex items-center justify-center p-6">
      <div className="rounded-xl border border-red-500/30 bg-red-500/[0.06] p-6 max-w-md text-center">
        ...
        <Link href="/app/campagnes" className="mt-4 inline-flex items-center gap-2 text-sm text-violet-400 hover:text-violet-300">
          <ArrowLeft size={14} />
          Retour
        </Link>
      </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            <Link
              href={`/app/campagnes/campaigns/new?list=${listId}`}
            ...
            {SMS_CAMPAIGNS_ENABLED && (
              <Link
                href={`/app/campagnes/sms/new?list=${listId}`}
