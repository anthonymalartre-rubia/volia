...
function NewSmsContent() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      router.push(`/app/campagnes/sms/${data.campaign.id}`);
    } catch {
  ...
  if (authState === 'no-admin') return <NoAdminScreen email={currentEmail} signOut={async () => { await supabase.auth.signOut(); router.push('/login?return=/app/campagnes/sms/new'); }} />;

  return (
    <div className="min-h-screen bg-surface-base text-content-primary p-4 sm:p-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <Link href="/app/campagnes/sms" className="inline-flex items-center gap-2 text-sm text-content-tertiary hover:text-emerald-400 transition mb-2">
            <ArrowLeft size={14} />
            Campagnes SMS
          </Link>
        ...
        {lists.length === 0 ? (
          <div className="rounded-2xl border border-amber-400 bg-amber-50 p-6 text-center">
            ...
            <Link href="/app/campagnes" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-violet-600 hover:bg-violet-500 text-white text-sm font-semibold transition">
              <Users size={14} />
              Aller aux listes
            </Link>
          </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex items-center gap-2 justify-end pt-2">
                <Link href="/app/campagnes/sms" className="px-3 py-2 rounded-lg text-sm text-content-secondary hover:text-content-primary transition">Annuler</Link>
                <button
                  type="submit"
  ...
  return (
    <div className="min-h-screen bg-surface-base flex items-center justify-center p-6">
      <div className="max-w-md w-full rounded-2xl border border-line bg-surface-card p-8 text-center">
        ...
        <Link href="/login?return=/app/campagnes/sms/new" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-sm font-semibold transition"><LogIn size={14} />Se connecter</Link>
      </div>
    </div>
  );
}
