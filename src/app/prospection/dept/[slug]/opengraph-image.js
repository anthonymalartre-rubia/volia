...
import { getDepartmentBySlug } from '@/lib/slugs';
...
export default async function Image({ params }) {
  ...
  const title = dept ? `${dept.name} (${dept.code})` : 'France';
    ...
    (
      <div
        style={{
          ...
          padding: '60px', fontFamily: 'system-ui, sans-serif', position: 'relative',
        }}
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', maxWidth: '900px' }}>
          ...
          <div style={{ fontSize: '64px', fontWeight: 700, color: 'white', lineHeight: 1.1, marginBottom: '24px' }}>
            Prospection B2B {title}
          </div>
          <div style={{ fontSize: '24px', color: '#a1a1aa', lineHeight: 1.4 }}>
            150+ secteurs couverts. Trouvez les emails des entreprises locales.
          </div>
        </div>
