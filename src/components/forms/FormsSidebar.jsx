...
// ─────────────────────────────────────────────────────────────────────
// FormsSidebar — sidebar contextuelle pour /admin/forms/*
// ─────────────────────────────────────────────────────────────────────
...
//   - /admin/forms                      → Mes formulaires (hub)
//   - /admin/forms/[id]                 → idem (édition)
//   - /admin/forms/templates            → Templates (placeholder F3)
//   - /admin/forms/stats                → Statistiques (placeholder F4)
// ─────────────────────────────────────────────────────────────────────
...
const NAV_ITEMS = [
  {
    ...
    matches: (p) =>
      p === '/admin/forms' ||
      p === '/admin/forms/' ||
      (p.startsWith('/admin/forms/') &&
        !p.startsWith('/admin/forms/templates') &&
        !p.startsWith('/admin/forms/stats')),
  },
  {
    id: 'templates',
    label: 'Templates',
  ...
  return (
    <>
      ...
      <aside
        ...
        `}
      >
        {/* Hint gradient pink au sommet */}
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-pink-500/40 to-transparent pointer-events-none" />
        ...
        <div className="flex flex-col h-full p-4">
          ...
          {/* Navigation */}
          <nav className="space-y-1" role="navigation" aria-label="Navigation Formulaires">
            {NAV_ITEMS.map((item) => {
              ...
              const className = `
                ...
                ${
                  isActive
                    ? 'bg-pink-100/80 text-pink-700 shadow-sm shadow-pink-500/10'
                    : isDisabled
                ...
                <>
                  ...
                  <div
                    className={`p-1.5 rounded-lg transition-colors ${
                      isActive ? 'bg-pink-200/60' : 'bg-surface-card'
                    }`}
                  >
                    <Icon size={16} />
                  </div>
                  <div className="flex-1 text-left">
                    ...
                    <div
                      className={`text-[10px] ${
                        isActive ? 'text-pink-700/60' : 'text-content-faint'
                      }`}
                    >
                      {item.description}
                    </div>
          ...
          <div className="mt-auto">
            <div className="relative p-4 rounded-xl bg-gradient-to-br from-pink-500/[0.06] to-rose-500/[0.06] border border-pink-500/15 overflow-hidden">
              <div className="absolute -bottom-8 -right-8 w-20 h-20 rounded-full bg-pink-500/15 blur-2xl pointer-events-none" />
              <div className="relative flex items-center gap-2 mb-2">
                <Sparkles size={11} className="text-pink-600" />
                <p className="text-[10px] uppercase tracking-wider text-pink-700 font-semibold">
                  Module Formulaires
                </p>
              </div>
              <p className="relative text-[10px] text-content-tertiary leading-relaxed">
                Forms multi-pages avec logique conditionnelle, bridge natif
                vers CRM et Campagnes. Tally-killer.
              </p>
            </div>
          </div>
        </div>
